...
public static class DBHomepage
{
    ...
    public static async Task<List<ProductItem>> RunAllAsync(string connStr)
    ...
    //    var listProduct = await DBHomepage.SelectAllProductsByBrand(connStr, "dell");
    ...
    public static async Task<List<ProductItem>> SelectAllProductsByBrand(string connStr, string brandName)
    {
        ...
        try
        {
            ...
            using var cmd = new SqlCommand(sql, conn);
            cmd.Parameters.Add(new SqlParameter("@brandName", SqlDbType.VarChar) { Value = brandName });
            using var reader = await cmd.ExecuteReaderAsync();
